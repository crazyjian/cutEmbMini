...
Page({
  ...
  search:function() {
    var obj = this;
    wx.request({
      url: app.globalData.backUrl + '/erp/minigetsampleinspectionbyemporder',
      data: {
      ...
      success: function (res) {
        if (res.statusCode == 200 && res.data) {
          obj.setData({
            records: res.data.sampleInspectionByEmpOrder,
          });
        }
      },
      fail:function() {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          ...
          wx.request({
            url: app.globalData.backUrl + '/erp/minideletesampleinspection',
            data: {
              sampleInspectionID: inspectionID
            },
            method: 'POST',
            header: {
              'content-type': 'application/x-www-form-urlencoded' // 默认值
